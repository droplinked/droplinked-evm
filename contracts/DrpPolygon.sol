...
contract Droplinked is ERC1155{
    ...
    function direct_buy(uint price, address recipient, uint80 roundId) public payable {
        ...
        // check the timestamp
        if (block.timestamp > timestamp && block.timestamp - timestamp > 2*heartBeat)
            revert oldPrice();
    ...
    function buy_recorded(address producer, uint token_id, uint shipping, uint tax, uint amount, uint80 roundId) public payable{
    ...
    function buy_affiliate(uint request_id, uint amount, uint shipping, uint tax, uint80 roundId) public payable{
